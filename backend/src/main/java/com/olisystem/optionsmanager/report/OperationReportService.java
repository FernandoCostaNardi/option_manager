...
public class OperationReportService {
  ...
  @Autowired private OperationReportData operationReportData;

  public byte[] generateExcelReport(OperationFilterCriteria filterCriteria) {
    OperationSummaryResponseDto summary =
        operationReportData.findByFilters(filterCriteria, Pageable.unpaged());
    ...
    try (XSSFWorkbook workbook = new XSSFWorkbook()) {
      ...
      for (OperationItemDto operation : operations) {
        ...
        row.createCell(11)
            .setCellValue(operation.getResult() != null ? operation.getResult().doubleValue() : 0);
  ...
  public byte[] generatePdfReport(OperationFilterCriteria filterCriteria) {
    ...
    try {
      ...
      com.itextpdf.text.Font titleFont =
          new com.itextpdf.text.Font(FontFamily.HELVETICA, 16, com.itextpdf.text.Font.BOLD);
      ...
      for (OperationItemDto operation : operations) {
        ...
        table.addCell(
            operation.getExitUnitPrice() != null ? operation.getExitUnitPrice().toString() : "");
