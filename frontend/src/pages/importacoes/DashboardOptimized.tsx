...
export function DashboardInvoiceProcessing() {
  // ===== ESTADOS PRINCIPAIS =====
  const [allInvoices, setAllInvoices] = useState<SimpleInvoiceData[]>([]);
  ...
  const loadAllInvoices = async () => {
    setLoading(true);
    try {
      // Carrega todas as invoices de uma só vez
      const response = await InvoiceProcessingService.getSimpleInvoices(0, 1000);
      setAllInvoices(response.content);
    } catch (error) {
  ...
  const totalValue = allInvoices.reduce((sum, inv) => sum + inv.grossOperationsValue, 0);
  const totalCosts = allInvoices.reduce((sum, inv) => sum + inv.totalCosts, 0);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {[
            ...
            <button
              ...
              onClick={() => {
                ...
                setCurrentPage(0); // Reset página ao trocar aba
              }}
      ...
      <ImportarNotaModal
        ...
        onSuccess={() => {
          setModalOpen(false);
          loadAllInvoices(); // Recarrega dados após importação
        }}
      />
    </div>
  );
